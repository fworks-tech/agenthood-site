...
describe("applyToolReplayOutcome", () => {
  it("flips a failed tool call to complete with a fresh result", () => {
    const next = applyToolReplayOutcome(base, "a1", "t2", { result: "fixed!" }, 2500);
  ...
  it("keeps a failed outcome as error with the message", () => {
    const next = applyToolReplayOutcome(base, "a1", "t2", { error: "Error: still broken" }, 3000);
  ...
  it("leaves unrelated tool calls and messages untouched", () => {
    const next = applyToolReplayOutcome(base, "a1", "t2", { result: "ok" }, 5000);
