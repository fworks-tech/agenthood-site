      ...
      async start(controller) {
        ...
        try {
          ...
          if (toolSchemas && toolSchemas.length > 0) {
            ...
            const finalText = await runToolLoop(provider, messages, toolSchemas, toolCallsRun, signal, controller);
            output = finalText;
  ...
  for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
    ...
    for (const tc of resp.toolCalls) {
      ...
      controller.enqueue(new TextEncoder().encode(
        JSON.stringify({ type: "tool_call", id: tc.id, name: tc.name, args }) + "\n",
      ));
