...
export default function StudioError({
  ...
  return (
    <div className="flex flex-1 min-h-0 items-center justify-center bg-zinc-50 dark:bg-zinc-950">
      <div className="text-center max-w-md px-6">
        ...
        {error.digest && (
          <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-600">Error ID: {error.digest}</p>
        )}
        <button
          onClick={reset}
          className="mt-4 rounded-md border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-4 py-2 text-sm text-zinc-700 dark:text-zinc-300 hover:border-zinc-400 dark:hover:border-zinc-500 transition-colors"
        >
          Try again
        </button>
      </div>
