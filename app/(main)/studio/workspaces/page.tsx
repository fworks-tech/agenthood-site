...
export default function WorkspacesPage() {
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      <div className="border-b border-zinc-200 dark:border-zinc-800 px-4 py-3 md:px-6">
        <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Workspaces</h1>
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          <aside className="hidden w-64 shrink-0 border-r border-zinc-200 dark:border-zinc-800 p-4 md:block">
            ...
            {isRunning ? (
              <button
                onClick={workspace.stop}
                className="mt-4 w-full cursor-pointer rounded-lg border border-zinc-200 dark:border-zinc-800 px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400 transition-all duration-200 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:border-zinc-300 dark:hover:border-zinc-700 hover:text-zinc-800 dark:hover:text-zinc-200 hover:scale-[1.01] active:scale-[0.99]"
              >
                Stop workspace
              </button>
            ) : (
              <button
                onClick={handleReset}
                className="mt-4 w-full cursor-pointer rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-800 dark:text-zinc-200 transition-all duration-200 hover:bg-zinc-200 dark:hover:bg-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-600 hover:text-black dark:hover:text-white hover:scale-[1.01] active:scale-[0.99]"
              >
                New workspace
              </button>
            )}
          ...
          <div className="flex flex-1 min-h-0 flex-col min-w-0">
            <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 px-4 py-2 md:hidden">
              <button onClick={() => setDrawerOpen(true)} className="cursor-pointer rounded border border-zinc-200 dark:border-zinc-800 px-3 py-1.5 text-sm text-zinc-700 dark:text-zinc-300 transition-all duration-200 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:border-zinc-300 dark:hover:border-zinc-700 hover:scale-[1.02] active:scale-95">
                Agents
              </button>
              <div className="flex items-center gap-2">
                <span className="text-xs text-zinc-500">{workspace.workspaceState}</span>
                {isRunning ? (
                  <button onClick={workspace.stop} className="cursor-pointer rounded border border-zinc-300 dark:border-zinc-700 px-2.5 py-1 text-xs text-zinc-700 dark:text-zinc-300 transition-all hover:bg-zinc-200 dark:hover:bg-zinc-800">
                    Stop
                  </button>
                ) : (
                  <button onClick={handleReset} className="cursor-pointer rounded border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-2.5 py-1 text-xs font-medium text-zinc-800 dark:text-zinc-200 transition-all hover:bg-zinc-200 dark:hover:bg-zinc-800">
                    New
                  </button>
                )}
              </div>
            ...
            <div className="border-t border-zinc-200 dark:border-zinc-800 p-4">
              <div className="mx-auto flex max-w-3xl gap-2">
                ...
                {isRunning ? (
                  ...
                  <button onClick={handleReset} className="cursor-pointer rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 transition-all duration-200 hover:bg-zinc-200 dark:hover:bg-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-600 hover:text-zinc-800 dark:hover:text-zinc-200 hover:scale-[1.02] active:scale-95">
                    New workspace
                  </button>
                )}
              </div>
      ...
      <MobileDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} onOpen={() => setDrawerOpen(true)}>
        <div className="p-4">
          ...
          {isRunning ? (
            ...
            <button onClick={() => { handleReset(); setDrawerOpen(false) }} className="mt-4 w-full cursor-pointer rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-800 dark:text-zinc-200 transition-all hover:bg-zinc-200 dark:hover:bg-zinc-800">
              New workspace
            </button>
          ) : null}
        </div>
