...
## For this repository (agenthood-site)

This is the marketing and documentation site for Agenthood, deployed to Vercel.
It is a consumer/presenter of the upstream `fworks-tech/agenthood` repo:

- `content/` is generated at build time from upstream `main` by
  `scripts/sync-docs.mjs` (docs, ADRs, academy) and `scripts/sync-skills.mjs`
  (member SKILL.md files plus `docs/members/registry.json`). Never edit files
  under `content/` directly — edit the upstream repo or the sync scripts.
- `content/news/` is tracked and human- or agent-written; the daily digest
  agent (`scripts/generate-news-digest.mjs`) drafts release digests there.

```bash
npm run sync          # refresh content/ from upstream (runs in prebuild)
npm run sync-skills   # regenerate studio agent data + registry metadata
npm run sync-news     # rebuild the news manifest
npm run dev           # local dev (runs predev sync in lenient mode)
npm run lint          # ESLint
npm test              # Vitest unit tests
npm run test:e2e      # Playwright end-to-end tests
npm run build         # production build (runs prebuild sync)
```

Local conventions mirror the Society's: conventional commits, one concern per
branch, tests before merge. The Studio adapter and API routes live under
`app/(main)/studio/` and `app/api/studio/`; see
[README.md](README.md) and `docs/adr/` for architecture decisions.
